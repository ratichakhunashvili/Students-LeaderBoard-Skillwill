...
function getQrImageUrl(token, size = 220) {
  const payload = makeActivityQrPayload(token);
...
function parseActivityQrPayload(rawValue) {
  ...
  if (value.startsWith('doable-activity:')) {
    return value.replace('doable-activity:', '').trim();
  }

  try {
    const parsed = JSON.parse(value);
    return parsed?.type === 'activity' ? String(parsed.token || '').trim() : '';
  } catch (_) {
    return '';
  }
}
    ...
    (snapshot) => {
      ...
      students.forEach((student) => {
        tbody.insertAdjacentHTML(
          ...
          <td>
            <button class="btn btn-sm btn-outline-primary me-2" onclick="editStudent('${student.id}', '${escapeAttr(student.name || "")}', '${escapeAttr(student.email || "")}')">
        ...
        grid.insertAdjacentHTML(
          ...
          <div class="activity-card">
            ...
            <div class="qr-box my-3 text-center">
              <img src="${getQrImageUrl(qrToken, 180)}" alt="QR code for ${escapeAttr(activity.name || 'activity')}" class="activity-qr-img" />
              <small class="d-block text-muted mt-2">Students scan this code to get points</small>
            </div>
            <div class="d-grid gap-2">
              <button class="btn btn-sm btn-gradient" onclick="downloadActivityQr('${escapeAttr(qrToken)}', '${escapeAttr(activity.name || 'activity')}')">Download QR</button>
              <button class="btn btn-sm btn-outline-danger" onclick="deleteActivity('${activity.id}')">Delete</button>
            </div>
          </div>
        </div>`,
        );
...
window.createActivity = async function () {
  ...
  try {
    ...
    await addDoc(collection(db, "activities"), {
      ...
      qrToken,
      qrPayload: makeActivityQrPayload(qrToken),
...
window.downloadActivityQr = function (token, name) {
  const safeName = String(name || 'activity')
    .toLowerCase()
    .replace(/[^a-z0-9]+/gi, '-')
    .replace(/^-|-$/g, '') || 'activity';
  const link = document.createElement('a');
  link.href = getQrImageUrl(token, 600);
...
async function handleActivityQrScan(decodedText) {
  ...
  const token = parseActivityQrPayload(decodedText);
  ...
  try {
    await awardActivityPointsFromQr(token);
...
async function awardActivityPointsFromQr(token) {
  const activitySnapshot = await getDocs(collection(db, 'activities'));
  const activity = getSnapshotDocs(activitySnapshot).find(
    (item) => item.qrToken === token || item.id === token,
  );
