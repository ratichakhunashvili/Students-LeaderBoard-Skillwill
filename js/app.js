...
function getSelectedLoginRole() {
  return (
    document.querySelector("#loginTabs .nav-link.active")?.dataset.role ||
    "student"
  );
}
...
onAuthStateChanged(auth, async (user) => {
  ...
  await loadUserData(user.uid, user.email, {
    createIfMissing: page !== "login.html",
    defaultRole: "student",
  });
...
window.viewStudentHistory = async function (studentId) {
  ...
  viewedStudent =
    allAdminStudents.find((student) => student.id === studentId) || null;
  if (!viewedStudent) {
  ...
  setText(
    "studentHistoryTitle",
    `${viewedStudent.name || "Student"} — Point History`,
  );
  setText(
    "studentHistorySubtitle",
    `${viewedStudent.email || ""} • ${viewedStudent.totalPoints || 0} total points`,
  );
  setValue("studentHistorySearchInput", "");
...
function renderStudentHistoryRecords() {
  ...
  const term = String(
    document.getElementById("studentHistorySearchInput")?.value || "",
  )
    .toLowerCase()
    .trim();
  const records = viewedStudentPointRecords.filter((point) => {
    if (!term) return true;
    return `${point.activityName || ""} ${point.source || ""} ${formatDate(point.createdAt)} ${point.points || ""}`
      .toLowerCase()
      .includes(term);
  });
  ...
  if (!records.length) {
    tbody.innerHTML =
      '<tr><td colspan="5" class="text-center text-muted py-4">No point records found.</td></tr>';
    return;
  }
...
function refreshActivityQrCodes() {
  ...
  renderedQrActivities.forEach((activity) => {
    const img = document.querySelector(
      `.activity-qr-img[data-activity-id="${CSS.escape(activity.id)}"]`,
    );
    const countdown = document.querySelector(
      `.qr-countdown[data-activity-id="${CSS.escape(activity.id)}"]`,
    );
  ...
  try {
    ...
    select.onchange = () => {
      ...
      if (pointAmount && selected?.dataset.points) {
        ...
        pointAmount.title =
          "Points are automatically taken from the selected activity.";
      }
    };
  } catch (error) {
...
async function assignActivityPointsToStudent(studentId, activityId) {
  ...
  showSuccess(
    `${amount} points assigned to ${studentData.name || "student"} for ${activityData.name || "activity"}.`,
  );
  return true;
}
...
function renderAdminPointLedger() {
  ...
  const term = String(
    document.getElementById("pointRecordsSearchInput")?.value || "",
  )
    .toLowerCase()
    .trim();
  const points = allAdminPointRecords
    .filter((point) => {
      if (!term) return true;
      return `${point.studentName || ""} ${point.activityName || ""} ${point.source || ""} ${formatDate(point.createdAt)} ${point.points || ""}`
        .toLowerCase()
        .includes(term);
    })
    .slice(0, 200);
  ...
  if (!points.length) {
    tbody.innerHTML =
      '<tr><td colspan="6" class="text-center text-muted py-4">No point records found</td></tr>';
    return;
  }
...
window.startQrScanner = async function () {
  ...
  if (!window.Html5Qrcode) {
    showError(
      "QR scanner library did not load. Check internet connection and refresh the page.",
    );
    return;
  }
  ...
  try {
    ...
    setText(
      "qrScanResult",
      "Camera is active. Point it at the activity QR code.",
    );
    ...
    showError(
      "Could not start camera. Allow camera permission and use HTTPS or localhost.",
    );
...
async function awardActivityPointsFromQr(qrData) {
  if (!qrData || qrData.version !== 2) {
    setText(
      "qrScanResult",
      "This QR code is old. Ask admin to show the live QR again.",
    );
  ...
  if (qrData.slot !== currentSlot) {
    setText(
      "qrScanResult",
      "This QR code expired. Please scan the live QR again.",
    );
    showError("This QR code expired. It changes every 30 seconds.");
    return;
  }

  const activityDocSnap = await getDoc(
    doc(db, "activities", qrData.activityId),
  );
  ...
  if (alreadyAwarded) {
    setText(
      "qrScanResult",
      `You already received points for ${activity.name || "this activity"}.`,
    );
    showError(
      `You already received points for ${activity.name || "this activity"}.`,
    );
  ...
  currentUserData.totalPoints =
    Number(currentUserData.totalPoints || 0) + amount;
  setText("myPoints", currentUserData.totalPoints);
  setText(
    "qrScanResult",
    `Success! You received ${amount} points for ${activity.name || "activity"}.`,
  );
  showSuccess(
    `Success! You received ${amount} points for ${activity.name || "activity"}.`,
  );
